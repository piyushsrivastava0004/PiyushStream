...
import { getPopularMovies, getTopRatedMovies, getNowPlayingMovies, getUpcomingMovies, getMovieGenres, discoverMovies } from '../api/tmdb'

const CATEGORIES = [
  { id: 'popular', label: 'Popular', fn: getPopularMovies },
  { id: 'top_rated', label: 'Top Rated', fn: getTopRatedMovies },
  { id: 'now_playing', label: 'Now Playing', fn: getNowPlayingMovies },
  { id: 'upcoming', label: 'Upcoming', fn: getUpcomingMovies },
]
...
export default function Movies() {
  ...
  useEffect(() => {
    getMovieGenres().then((r) => setGenres(r.data.genres || []))
  }, [])
  ...
  const fetchMovies = useCallback(async () => {
    ...
    try {
      let res
      if (selectedGenre) {
        res = await discoverMovies({ with_genres: selectedGenre, page })
      } else {
